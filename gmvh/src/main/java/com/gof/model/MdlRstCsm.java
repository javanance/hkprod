...
//				Loss Ratio put to Map!!!!
				if(aa.getLossStep()!=null ) {
					log.info("in lossRatio :  {},{},{},{},{}", gocId, fulfillAmt, tempRst.getCalcCsmAmt(), aa.getLossStep(),  tempRst.getLossRatio());
...
//		rstBoxList.forEach(s-> log.info("aaa : {}", s.toString()));
		...
		for(Map.Entry<MstRunset, Double> entry : runsetRst.entrySet()) {
			log.info("entry : {},{},{},{},{},{}", gocId, journal.getRollFwdType(), journal.getCalcId(),entry.getKey(), entry.getValue(), lossRatio);
...
//				boxValue = csmInt(initClose);
		...
		case CSM_REVERSAL:
			...
			double moveDeltaCsm = priorClose.getDeltaCsmAmt();
...
//			boxValue = csmAdj(moveDeltaAmt, moveCloseAmt);
			boxValue = csmAdj(moveDeltaCsm, moveCloseAmt);
			coaValue = PrvdAcct.getCoaValue(ECoa.CSM, journal, boxValue);
