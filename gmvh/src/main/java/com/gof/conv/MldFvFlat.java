...
public class MldFvFlat {
	...
	public static Stream<FvFlat> create(){
		...
		Map<String, Double> tovgMap = RawDao.getRawTvogGoc(bssd).stream()
											.filter(s->s.getRsDivId().equals(fvRsDivId))
											.collect(toMap(RawTvogGoc::getGocId, RawTvogGoc::getTvogAmt));
		
		Map<String, Double> raMap 	= RawDao.getRawRaGoc(bssd).stream()
											.filter(s->s.getLiabType().equals(ELiabType.LRC))
											.filter(s->s.getRsDivId().equals(fvRsDivId))
											.collect(toMap(RawRaGoc::getGocId, RawRaGoc::getRaAmt));
	...
	public static FvFlat build(String bssd,  String gocId,  double epvAmt, double raAmt, double tvogAmt) {
