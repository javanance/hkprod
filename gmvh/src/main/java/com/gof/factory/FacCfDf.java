...
//					log.info("aaa :{},{},{}", df, cf.getCfMonthNum(), cf.getGocId(), aa.getRunsetId());
					prevRate =  df.getPrevRate(); 
				}
...
//								.initRate(df.getInitRate())
								.initRate(getInitRate(cf.getLiabType(), df))
//								.prevRate(df.getPrevRate())							
								.prevRate(prevRate)							
...
//						}
	...
	private static double getInitRate(ELiabType liabType, DfLv3Flat df) {
		double initRate =0.0;
		if(initCurveYn.equals("Y")) {
			initRate = df.getInitRate();
		}
		else {
			if(currEirType.equals("EACH")) {
				initRate = liabType.equals(ELiabType.LRC)? df.getInitLrcEir(): df.getInitLicEir();
			}
			else if(currEirType.equals("LRC")) {
				initRate =  df.getInitLrcEir();
			}
			else {
				initRate =  df.getInitEir();
			}
		}	
		return initRate;
	}
