...
public class Job50_TransitionMatrixConvert {
	public static List<CorpCrdGrdPd> createCorpPdFromUd(String bssd) {
		List<CorpCrdGrdPd> rstList = new ArrayList<CorpCrdGrdPd>();
		CorpCrdGrdPd temp;

		List<TransitionMatrixUd> tmUd = TransitionMatrixDao.getDefaultRateUd(bssd);
//		rst.forEach(s -> log.info("zzz  : {}", s.getFromGrade()));

//		Map<String, Double> volMap = getVolMap(bssd);

		for (TransitionMatrixUd aa : tmUd) {
...
//			temp.setVol(volMap.getOrDefault(temp.getCrdGrdCd(), 0.0));
	...
	public static List<CorpCumPd> createCorpCumPdFromUd(String bssd) {
		List<CorpCumPd> rstList = new ArrayList<CorpCumPd>();
		CorpCumPd temp;

		List<TransitionMatrixUd> rst = TransitionMatrixDao.getTMUd(bssd);
