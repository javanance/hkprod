...
//	private static Map<String, GocRst> prevGocRstMap = new HashMap<String, GocRst>();
//	private static List<JournalBoxMapping> boxMap = new ArrayList<JournalBoxMapping>();
//	private static Map<EEventType, JournalBoxMapping> csmBoxMap = new HashMap<EEventType, JournalBoxMapping>();
...
//		GmvConstant.GROUP_TENOR_LIST 		= new ArrayList<Integer>(Arrays.asList(61, 120, 240, 360));
		...
		GmvConstant.CALC_TYPE_CSM_INT		= properties.getOrDefault("CALC_TYPE_CSM_INT", "CSM_INT_FACTOR").toString();
...
//		GmvConstant.RUNSET_FIRST_NEW		= properties.getOrDefault("RUNSET_FIRST_NEW",	"FIRST_201-1").toString();
//		GmvConstant.RUNSET_SECOND_NEW		= properties.getOrDefault("RUNSET_SECOND_NEW", "SECOND_201-1").toString();
//		GmvConstant.RUNSET_THIRD_NEW		= properties.getOrDefault("RUNSET_THIRD_NEW", "THIRD_201-1").toString();
//		GmvConstant.RUNSET_PREV				= properties.getOrDefault("RUNSET_PREV", "PREV_109-2").toString();	
//		GmvConstant.RUNSET_CURR				= properties.getOrDefault("RUNSET_CURR", "CURR_109-2").toString();
//		GmvConstant.DELTA_GROUP_PREV		= "RUN_PREV_SETTLE";
		
		
		
//		Map<ERsDivType, MstRunset> runsetMap = MstDao.getMstRunset().stream().filter(s->s.getCoaId().equals(ECoa.EPV))
//					.collect(toMap(MstRunset::getRsDivType, Function.identity(), (s,u)->s));
		
//		Map<ERsDivType, MstRunset> runsetMap = PrvdMst.getMstRunsetList(ECoa.EPV).stream()
//													  .collect(toMap(MstRunset::getRsDivType, Function.identity(), (s,u)->s));
//		
//		Map<String, MstRunset> deltaCfMap 	 = MstDao.getMstRunset().stream().filter(s->s.getCoaId().equals(ECoa.EPV))
//																	.collect(toMap(MstRunset::getDeltaGroup, Function.identity(), (s,u)->s));

//		GmvConstant.RUNSET_PREV				 = runsetMap.get(ERsDivType.PREV_CLOSING).getRunsetId();
//		GmvConstant.RUNSET_FIRST_NEW		 = runsetMap.get(ERsDivType.FIRST_NEW).getRunsetId();
//		GmvConstant.RUNSET_SECOND_NEW		 = runsetMap.get(ERsDivType.SECOND_NEW).getRunsetId();
//		GmvConstant.RUNSET_THIRD_NEW		 = runsetMap.get(ERsDivType.THIRD_NEW).getRunsetId();
//		GmvConstant.DELTA_GROUP_PREV		 = runsetMap.get(ERsDivType.PREV_CLOSING).getDeltaGroup();		//RUN_PREV_SETTLE
		
//		TODO ::AAAAA for GMV
//		GmvConstant.DELTA_GROUP_LOSS		 = runsetMap.get(ERsDivType.PUB_CLOSING).getDeltaGroup();		//RUN_PUB_SETTLE

//		GmvConstant.EIR_CF_RUNSET			 = runsetMap.get(ERsDivType.FINC_CHANGE).getRunsetId();
//		String priorEirDeltaGroup	 		 = runsetMap.get(ERsDivType.FINC_CHANGE).getPriorDeltaGroup();
//		GmvConstant.EIR_TARGET_RUNSET		 = deltaCfMap.get(priorEirDeltaGroup).getRunsetId();

//		GmvConstant.RUNSET_CURR				 = runsetMap.get(ERsDivType.CURR_CLOSING).getRunsetId();
		
//		List<MstRunset> runsetList = MstDao.getMstRunset().stream().filter(s->s.getCoaType().equals(ECoa.EPV)).collect(toList());
//		
//		for(MstRunset aa : runsetList) {
//			if(aa.getPrevYn().isTrueFalse() && aa.getSlidingNum()==0) {
//				GmvConstant.RUNSET_PREV = aa.getRunsetId();
//			}
//			else if (!aa.getPrevYn().isTrueFalse() && aa.getPriorDeltaGroup()==null) {
//				GmvConstant.RUNSET_CURR = aa.getRunsetId();
//			}
//		}
		
//		GmvConstant.DELTA_CF_PREV				= "RUN_PREV_SETTLE";
//		Map<String, String> rsDivMap = MstDao.getMstRsDiv().stream().collect(toMap(MstRsDiv::getRsType, MstRsDiv::getRsDivId));
//		GmvConstant.NEW_RS_DIV_ID 			=rsDivMap.getOrDefault("INIT_REC", "201-1");
//		GmvConstant.CLOSING_RS_DIV_ID		=rsDivMap.getOrDefault("CLOSING", "109-2");
//		GmvConstant.EIR_TARGET_RS_DIV_ID	=rsDivMap.getOrDefault("ACUT_CHANGE", "106-1");
//		GmvConstant.EIR_CF_RS_DIV_ID		=rsDivMap.getOrDefault("FIN_CHANGE", "108-1");
