...
public class FacBoxGoc {
	...
	public static List<RstBoxGoc> build(String bssd, ElLv2Delta box, List<MapRunsetCalc> runsetList){
		List<RstBoxGoc> rstList = new ArrayList<RstBoxGoc>();
		double boxValue =0.0;
		for(MapRunsetCalc calc : runsetList) {
			boxValue = calc.getMstCalc().getSignAdjust().getAdj() * box.getDeltaElAmt();
			//TODO : Change column!!!!!
			rstList.add(build(bssd, box.getGocId(), ELiabType.LRC, calc.getMstRunset(), calc.getCalcId(), boxValue));
		}
		return rstList;
...
//	public static List<RstBoxGoc> build(String bssd, LossRcvLv2Delta box, List<MapRunsetCalc> runsetList){
//		List<RstBoxGoc> rstList = new ArrayList<RstBoxGoc>();
//		double boxValue =0.0;
//		for(MapRunsetCalc calc : runsetList) {
//			boxValue = calc.getMstCalc().getSignAdjust().getAdj() * box.getDeltaLossRcvAmt();
//			//TODO : Change column!!!!!
//			rstList.add(build(bssd, box.getGocId(), ELiabType.LRC, calc.getMstRunset(), calc.getCalcId(), boxValue));
//		}
//		return rstList;
//	}
	...
	public static RstBoxGoc build(String bssd, RstLossRcv lr){
		
		double boxValue =0.0;
		
		boxValue = lr.getBoxValue();

		return build(bssd, lr.getGocId(), lr.getLiabType(), lr.getMstRunset(), lr.getCalcId(), boxValue);
