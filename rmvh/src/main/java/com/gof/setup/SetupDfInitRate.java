...
public class SetupDfInitRate {
	...
	public static Stream<DfLv2InitRate> createConversion(){
		...
		List<MstGoc> gocMstList = PrvdMst.getGocList().stream().filter(s->DateUtil.isGreaterOrEqual(stBssd, s.getInitYymm())).collect(toList());
		
		Map<String, Map<ELiabType, Double>> eirMap 
							= DfDao.getEir(stBssd).stream().collect(groupingBy(DfLv4Eir::getGocId, toMap(DfLv4Eir::getLiabType,DfLv4Eir::getEir)));
...
//	}
	...
	public static Stream<DfLv2InitRate> createNew(String gocId){
		...
		return build(gocId, initCurveYymm, getIrCurve(initCurveYymm), eir, eir).stream();
	}
	...
	private static List<DfLv2InitRate> build(String gocId, String initCurveYymm, List<DfLv1CurrRate> irCurve, double eir, double licEir) {
		List<DfLv2InitRate> rstList = new ArrayList<DfLv2InitRate>();
